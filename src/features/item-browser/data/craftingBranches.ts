...
export type CraftingBranchKind =
  | "introduction"
  | "specialization"
  | "collection";
...
const WEAPON_LINE_STATION: Readonly<
  Record<keyof typeof WEAPON_FAMILIES, CraftingStationGroup>
> = {
  sword: "warrior_forge",
  axe: "warrior_forge",
  mace: "warrior_forge",
...
const WEAPON_BRANCHES: Readonly<Record<CraftingBranchId, BranchDefinition>> = {
  ...
  weapon_sword: createWeaponBranch(
    "sword",
    "Sword Crafter",
    "Espadas y sus variantes",
  ),
  weapon_axe: createWeaponBranch(
    "axe",
    "Axe Crafter",
    "Hachas y sus variantes",
  ),
  weapon_mace: createWeaponBranch(
    "mace",
    "Mace Crafter",
    "Mazas y sus variantes",
  ),
  weapon_hammer: createWeaponBranch(
    "hammer",
    "Hammer Crafter",
    "Martillos y sus variantes",
  ),
  ...
  weapon_spear: createWeaponBranch(
    "spear",
    "Spear Crafter",
    "Lanzas y sus variantes",
  ),
  ...
  weapon_dagger: createWeaponBranch(
    "dagger",
    "Dagger Crafter",
    "Dagas y sus variantes",
  ),
...
const CATEGORY_DEFINITIONS: Readonly<
  Record<BranchCategory, BranchCategoryDefinition>
> = {
  armor: {
    category: "armor",
    introText:
...
export function isBranchCategory(
  category: ItemCategory,
): category is BranchCategory {
  return (
    category === "weapon" ||
    category === "armor" ||
    category === "offhand" ||
...
function isVisibleCraftingItem(
  item: Item,
  parsed: ParsedTieredItemId,
): boolean {
...
function getWeaponBranchId(
  parsed: ParsedTieredItemId,
): CraftingBranchId | null {
...
function findFamilyGroup<
  T extends Readonly<Record<string, ReadonlySet<string>>>,
>(groups: T, familyId: string): keyof T | null {
...
function getAccessoryBranchId(
  ...
  if (
    (parsed.familyId === "BAG" || parsed.familyId === "CAPE") &&
    parsed.tier === 2
  ) {
  ...
  const branchByLine: Readonly<
    Record<keyof typeof ACCESSORY_FAMILIES, CraftingBranchId>
  > = {
    bag: "accessory_bag",
    cape: "accessory_cape",
...
function getFamilyName(items: readonly Item[]): string {
  ...
  return representative
    ? representative.name.replace(TIER_NAME_SUFFIX, "")
    : "Familia sin nombre";
}
...
function buildFamilies(
  ...
  return Array.from(grouped.entries())
    .map(([id, familyItems]): ItemFamily => {
      const sortedItems = [...familyItems].sort(
      ...
      if (!representativeItem)
        throw new Error(`La familia ${id} no contiene ítems`);
    ...
    .sort((left, right) => {
      const orderDifference =
        getFamilyOrder(left.suffix) - getFamilyOrder(right.suffix);
...
export function buildCategoryCraftingCatalog(
  ...
  const sections = definition.sectionOrder.flatMap((stationGroup) => {
    const sectionBranches = branches.filter(
      (branch) => branch.stationGroup === stationGroup,
    );
...
export function isGroupedCraftingItem(
  category: BranchCategory,
  item: Item,
): boolean {
  const parsed = parseTieredItemId(item);
  return (
    parsed !== null &&
    isVisibleCraftingItem(item, parsed) &&
    getBranchId(category, parsed) !== null
  );
}

// Compatibilidad con los nombres usados por las pruebas y componentes previos.
export function buildArmorCraftingBranches(
  items: readonly Item[],
): readonly CraftingBranch[] {
  return buildCategoryCraftingCatalog("armor", items).branches;
}

export function buildArmorSearchFamilies(
  items: readonly Item[],
): readonly ItemFamily[] {
