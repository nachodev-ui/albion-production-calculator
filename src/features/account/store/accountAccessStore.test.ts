...
describe('account access entitlements', () => {
  it('uses the Free entitlement baseline for anonymous users', () => {
    ...
    expect(entitlementIsEnabled(null, ENTITLEMENT_KEYS.optimizerLiquidity)).toBe(
      false,
    )
  ...
  it('overrides the Free baseline with effective API entitlements', () => {
    ...
    expect(entitlementIsEnabled(PRO_ACCESS, ENTITLEMENT_KEYS.optimizerLiquidity)).toBe(
      true,
    )
